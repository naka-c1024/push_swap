...
/*   Updated: 2022/02/09 13:32:05 by ynakashi         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
int	case_three_sort(t_stack **a, t_stack **b)
{
	...
	if (first > second && first > third) // 大
	{
		if (second > third) // 大中小
			return (1);
		else if (second < third) // 大小中
			return (2);
	}
	else if (first < second && first < third) // 小中大
		return (3);
	else // 中
	{
		if (second > third) // 中大小
			return (4);
		else if (second < third) // 中小大
			return (5);
	}
	return (0);
}
